...
def resetGame():
    ...
    score = 0
    displayed_word_color = ''
    game_score.config(text = "Great !! Your Score : " + str(score))
...
def startCountDown():
    ...
    if(timer >= 0):
        ...
        if (timer == -1):
            time_left.config(text="Opps ! Game Over !!!")
...
my_window = Tk()
my_window.title("Python Mini Project Color Game")
my_window.geometry("500x200")

app_font = font.Font(family='Helvetica', size = 12)

game_desp = "Game Description : Enter the Color of the Word displayed below not that Word... \n if u don't understand read again !! All the Best !! \n 19DIT019 - KIRTAN GOHIL"
myFont = font.Font(family='Helvetica')

game_description = Label(my_window, text = game_desp, font = app_font, fg= "orange")
game_description.pack()

game_score = Label(my_window, text = "Your Score : " + str(score), font = (font.Font(size=16)), fg = "green")
game_score.pack()

display_words = Label(my_window , font = (font.Font(size=28)), pady = 20)
display_words.pack()

time_left = Label(my_window, text = "Game Ends in : -", font = (font.Font(size=14)), fg = "red")
time_left.pack()
...
start_button = Button(btn_frame, text = "Start", width = 20, fg = "black", bg = "light blue", bd = 0,padx = 20, pady = 10 , command = startGame)
start_button.grid(row=0, column= 0)

reset_button = Button(btn_frame, text = "Reset", width = 20, fg = "black", bg = "pink", bd = 0,padx = 20, pady = 10 , command = resetGame)
